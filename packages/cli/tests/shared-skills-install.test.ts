...
const FULLSTACK = ['fullstack-contract'];
...
test('install puts the 7 frontend, 8 shared, 10 backend and 1 fullstack skills in the project, and a re-run is a no-op', async () => {
  ...
  try {
    ...
    for (const name of [...FRONTEND, ...SHARED, ...BACKEND, ...FULLSTACK]) assert.ok(existsSync(join(projectRoot, '.claude', 'skills', name, 'SKILL.md')), name);
    assert.equal(readdirSync(join(projectRoot, '.claude', 'skills'), { withFileTypes: true }).filter((e) => e.isDirectory()).length, 26);
    assert.ok(existsSync(join(projectRoot, '.claude', 'skills', 'fullstack-contract', 'references', 'contract-format.md')));
