...
import { DEFAULT_KNOWLEDGE_DIR, writeKnowledge, type KnowledgeName } from './repo-memory.js';
...
/** Write the rendered knowledge under the knowledge directory, stamped with HEAD. Derived data: regenerated on every run. */
export function writeRepoKnowledge(root: string, index: RepoIndex, knowledgeDir: string = DEFAULT_KNOWLEDGE_DIR): { written: KnowledgeName[]; sourceSha: string } {
  if (index.headSha === null) throw new Error('repo index --write needs a git repository with at least one commit');
  const bodies = renderKnowledge(index);
  const written: KnowledgeName[] = [];
  for (const [name, body] of Object.entries(bodies) as Array<[KnowledgeName, string]>) {
    writeKnowledge(root, name, body, { sourceSha: index.headSha }, knowledgeDir);
    written.push(name);
  }
  return { written, sourceSha: index.headSha };
}
