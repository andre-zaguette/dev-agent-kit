...
`TIMESTAMP` is stored in UTC and limited to 2038; `DATETIME` is not converted, so decide which one you mean and store instants in UTC. Index column order follows the query (equality first, then range, then sort). Avoid `SELECT *` and deep `OFFSET`; page with `WHERE id > ? ORDER BY id LIMIT`. The default isolation level is `REPEATABLE READ`; use `SELECT ... FOR UPDATE` for contended rows. If the requested online algorithm is not supported, MySQL raises an error instead of blocking, which is the safe behavior: fall back to a tool such as `gh-ost` or `pt-online-schema-change` on large tables.
